...
def userHome(request):
    ...
    posts = Post.objects.filter(user__in=followed_users).order_by(
        '-pk') | Post.objects.filter(user=request.user).order_by('-pk')
    liked_ = [i for i in posts if Like.objects.filter(
...
def comment(request, username, id):
    ...
    return render(request, 'userpage/comments.html', {"post": post, "comments": comments, 'comment_form': comment_form})
